...
if [ $GPU_ID == "reset" ]; then
    ...
    # echo "ssh -A $SERVER \"EXP_DIR=${EXP_DIR} bash -s\" < exp_scripts/execute_on_server.sh $GPU_ID $N_REPEAT $PARAMS &"
    # echo < exp_scripts/execute_on_server.sh $GPU_ID $N_REPEAT $PARAMS
