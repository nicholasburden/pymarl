...
class RNNConvDDPGInputGridAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        in_channels, n_dim_x, n_dim_y = input_shape["2d"][0]
        ...
        # find output dim
        dummy_th = th.zeros(1, in_channels, *input_shape["2d"][0][1:]).to(next(self.parameters()).device)
        out = self.conv2(self.conv1(dummy_th))
    ...
    def forward(self, inputs, hidden_state):
        y = F.elu(self.conv1(th.cat([inputs["2d"][0], inputs["actions_2d"]], dim=-3)))
        y = F.elu(self.conv2(y))
